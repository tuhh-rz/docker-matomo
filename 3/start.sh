...
rsync -rc /tmp/matomo/* "/var/www/html"
chown -Rf www-data.www-data "/var/www/html"

perl -i -ne 'print unless /^# INJECTION MARKER START/ .. /^# INJECTION MARKER END/' /etc/crontab
echo '# INJECTION MARKER START' >> /etc/crontab
echo "5 * * * * www-data /usr/bin/php /var/www/html/console core:archive --url=${MATOMO_BASE_URL}" >> /etc/crontab
echo '# INJECTION MARKER END' >> /etc/crontab
